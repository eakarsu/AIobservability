...
// Gap (AI): No explicit '/anomaly
// No explicit /anomaly-detect endpoint exposed in routers
import React, { useState } from 'react';

export default function GapNoExplicitAnomalyPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-explicit-anomaly/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No explicit '/anomaly</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No explicit /anomaly-detect endpoint exposed in routers</p>
