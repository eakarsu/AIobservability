...
// Custom Feature: OpenTelemetry compatibility
// OpenTelemetry compatibility: Accept OTLP gRPC/HTTP so customers can use existing collectors
import React, { useState } from 'react';

export default function CFOpentelemetryCompatibilityPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-opentelemetry-compatibility/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>OpenTelemetry compatibility</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>OpenTelemetry compatibility: Accept OTLP gRPC/HTTP so customers can use existing collectors</p>
