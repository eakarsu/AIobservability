...
// Gap (AI): No '/root
// No /root-cause-analyze for traces
import React, { useState } from 'react';

export default function GapNoRootPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-root/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No '/root</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No /root-cause-analyze for traces</p>
