...
// Custom Feature: LLM cost & latency dashboards
// LLM cost & latency dashboards: Per-model, per-route, per-user cost rollups with budget alerts
import React, { useState } from 'react';

export default function CFLlmCostLatencyDashboardsPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-llm-cost-latency-dashboards/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>LLM cost & latency dashboards</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>LLM cost & latency dashboards: Per-model, per-route, per-user cost rollups with budget alerts</p>
