...
// Prompt regression detector: Diff prompt versions; auto-evaluate against golden set; flag regressions before deploy
import React, { useState } from 'react';

export default function CFPromptRegressionDetectorPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-prompt-regression-detector/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Prompt regression detector</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Prompt regression detector: Diff prompt versions; auto-evaluate against golden set; flag regressions before deploy</p>
