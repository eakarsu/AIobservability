...
// Custom Feature: Eval harness integration
// Eval harness integration: Plug eval suites (RAGAS, HELM-style) into the same pipeline as traces
import React, { useState } from 'react';

export default function CFEvalHarnessIntegrationPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-eval-harness-integration/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Eval harness integration</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Eval harness integration: Plug eval suites (RAGAS, HELM-style) into the same pipeline as traces</p>
