...
// Gap (AI): No '/drift
// No /drift-detect (model/data drift monitoring)
import React, { useState } from 'react';

export default function GapNoDriftPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-drift/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No '/drift</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No /drift-detect (model/data drift monitoring)</p>
