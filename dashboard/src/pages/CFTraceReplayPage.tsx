...
// Custom Feature: Trace replay
// Trace replay: Replay production traces against new model/prompt to measure delta in quality and cost
import React, { useState } from 'react';

export default function CFTraceReplayPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-trace-replay/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Trace replay</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Trace replay: Replay production traces against new model/prompt to measure delta in quality and cost</p>
