...
async def _call_llm(system_prompt: str, user_prompt: str) -> dict:
    ...
    headers = {
        ...
        'X-Title': 'AIobservability / gap-no-cost',
    }
...
@router.post('/gap-no-cost/run', tags=['Gap Features'])
async def run_gap_no_cost(body: RunInput) -> dict:
    sys_p = (
        "You are an expert assistant for the gap feature: 'No `/cost'.\n"
        "Project: AIobservability.\n"
        "Context: No /cost-attribution-ai (per-model spend analysis)\n"
...
@router.get('/gap-no-cost/history', tags=['Gap Features'])
async def history_gap_no_cost() -> dict:
    return {'items': []}
