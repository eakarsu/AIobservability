...
async def _call_llm(system_prompt: str, user_prompt: str) -> dict:
    ...
    headers = {
        ...
        'X-Title': 'AIobservability / gap-no-root',
    }
...
@router.post('/gap-no-root/run', tags=['Gap Features'])
async def run_gap_no_root(body: RunInput) -> dict:
    sys_p = (
        "You are an expert assistant for the gap feature: 'No `/root'.\n"
        "Project: AIobservability.\n"
        "Context: No /root-cause-analyze for traces\n"
...
@router.get('/gap-no-root/history', tags=['Gap Features'])
async def history_gap_no_root() -> dict:
    return {'items': []}
