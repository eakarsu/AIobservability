"""
Gap Feature: No billing/usage metering
...
async def _call_llm(system_prompt: str, user_prompt: str) -> dict:
    ...
    headers = {
        ...
        'X-Title': 'AIobservability / gap-no-billing-usage-metering',
    }
...
@router.post('/gap-no-billing-usage-metering/run', tags=['Gap Features'])
async def run_gap_no_billing_usage_metering(body: RunInput) -> dict:
    sys_p = (
        "You are an expert assistant for the gap feature: 'No billing/usage metering'.\n"
        "Project: AIobservability.\n"
        "Context: No billing/usage metering\n"
    ...
    return {
        'ok': True,
        'feature': 'no-billing-usage-metering',
        'result': out['content'],
...
@router.get('/gap-no-billing-usage-metering/history', tags=['Gap Features'])
async def history_gap_no_billing_usage_metering() -> dict:
    return {'items': []}
