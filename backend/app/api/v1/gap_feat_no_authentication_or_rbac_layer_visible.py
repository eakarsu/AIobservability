"""
Gap Feature: No authentication or RBAC layer visible
No authentication or RBAC layer visible
...
async def _call_llm(system_prompt: str, user_prompt: str) -> dict:
    ...
    headers = {
        ...
        'X-Title': 'AIobservability / gap-no-authentication-or-rbac-layer-visible',
    }
...
@router.post('/gap-no-authentication-or-rbac-layer-visible/run', tags=['Gap Features'])
async def run_gap_no_authentication_or_rbac_layer_visible(body: RunInput) -> dict:
    sys_p = (
        "You are an expert assistant for the gap feature: 'No authentication or RBAC layer visible'.\n"
        "Project: AIobservability.\n"
        "Context: No authentication or RBAC layer visible\n"
    ...
    return {
        'ok': True,
        'feature': 'no-authentication-or-rbac-layer-visible',
        'result': out['content'],
...
@router.get('/gap-no-authentication-or-rbac-layer-visible/history', tags=['Gap Features'])
async def history_gap_no_authentication_or_rbac_layer_visible() -> dict:
    return {'items': []}
