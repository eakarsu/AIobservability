...
Per-model, per-route, per-user cost rollups with budget alerts
...
async def _call_llm(system_prompt: str, user_prompt: str) -> dict:
    ...
    headers = {
        ...
        'X-Title': 'AIobservability / cf-llm-cost-latency-dashboards',
    }
...
@router.post("/cf-llm-cost-latency-dashboards/run", tags=["Custom Features"])
async def run_feature(payload: RunPayload):
    system_prompt = (
        "You are an expert assistant helping with the feature: 'LLM cost & latency dashboards'. "
        "Context: Per-model, per-route, per-user cost rollups with budget alerts "
