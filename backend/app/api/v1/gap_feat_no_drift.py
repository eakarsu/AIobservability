...
async def _call_llm(system_prompt: str, user_prompt: str) -> dict:
    ...
    headers = {
        ...
        'X-Title': 'AIobservability / gap-no-drift',
    }
...
@router.post('/gap-no-drift/run', tags=['Gap Features'])
async def run_gap_no_drift(body: RunInput) -> dict:
    sys_p = (
        "You are an expert assistant for the gap feature: 'No `/drift'.\n"
        "Project: AIobservability.\n"
        "Context: No /drift-detect (model/data drift monitoring)\n"
...
@router.get('/gap-no-drift/history', tags=['Gap Features'])
async def history_gap_no_drift() -> dict:
    return {'items': []}
