...
async def _call_llm(system_prompt: str, user_prompt: str) -> dict:
    ...
    headers = {
        ...
        'X-Title': 'AIobservability / gap-no-audit-logging',
    }
...
@router.post('/gap-no-audit-logging/run', tags=['Gap Features'])
async def run_gap_no_audit_logging(body: RunInput) -> dict:
    sys_p = (
        "You are an expert assistant for the gap feature: 'No audit logging'.\n"
        "Project: AIobservability.\n"
...
@router.get('/gap-no-audit-logging/history', tags=['Gap Features'])
async def history_gap_no_audit_logging() -> dict:
    return {'items': []}
