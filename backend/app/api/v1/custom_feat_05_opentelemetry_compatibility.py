...
Accept OTLP gRPC/HTTP so customers can use existing collectors
...
async def _call_llm(system_prompt: str, user_prompt: str) -> dict:
    ...
    headers = {
        ...
        'X-Title': 'AIobservability / cf-opentelemetry-compatibility',
    }
...
@router.post("/cf-opentelemetry-compatibility/run", tags=["Custom Features"])
async def run_feature(payload: RunPayload):
    system_prompt = (
        "You are an expert assistant helping with the feature: 'OpenTelemetry compatibility'. "
        "Context: Accept OTLP gRPC/HTTP so customers can use existing collectors "
