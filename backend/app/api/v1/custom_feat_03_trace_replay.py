...
Replay production traces against new model/prompt to measure delta in quality and cost
...
async def _call_llm(system_prompt: str, user_prompt: str) -> dict:
    ...
    headers = {
        ...
        'X-Title': 'AIobservability / cf-trace-replay',
    }
...
@router.post("/cf-trace-replay/run", tags=["Custom Features"])
async def run_feature(payload: RunPayload):
    system_prompt = (
        "You are an expert assistant helping with the feature: 'Trace replay'. "
        "Context: Replay production traces against new model/prompt to measure delta in quality and cost "
