...
Plug eval suites (RAGAS, HELM-style) into the same pipeline as traces
...
async def _call_llm(system_prompt: str, user_prompt: str) -> dict:
    ...
    headers = {
        ...
        'X-Title': 'AIobservability / cf-eval-harness-integration',
    }
...
@router.post("/cf-eval-harness-integration/run", tags=["Custom Features"])
async def run_feature(payload: RunPayload):
    system_prompt = (
        "You are an expert assistant helping with the feature: 'Eval harness integration'. "
        "Context: Plug eval suites (RAGAS, HELM-style) into the same pipeline as traces "
