"""
Gap Feature: No explicit `/anomaly
...
async def _call_llm(system_prompt: str, user_prompt: str) -> dict:
    ...
    headers = {
        ...
        'X-Title': 'AIobservability / gap-no-explicit-anomaly',
    }
...
@router.post('/gap-no-explicit-anomaly/run', tags=['Gap Features'])
async def run_gap_no_explicit_anomaly(body: RunInput) -> dict:
    sys_p = (
        "You are an expert assistant for the gap feature: 'No explicit `/anomaly'.\n"
        "Project: AIobservability.\n"
        "Context: No explicit /anomaly-detect endpoint exposed in routers\n"
...
@router.get('/gap-no-explicit-anomaly/history', tags=['Gap Features'])
async def history_gap_no_explicit_anomaly() -> dict:
    return {'items': []}
